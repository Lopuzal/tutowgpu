    ...
    async fn new(window: Window) -> Self {
        ...
        let (device, queue) = adapter.request_device(
            &wgpu::DeviceDescriptor {
                ...
                // WebGL doesn't support all of wgpu's features, so if we're building for the web
...
pub async fn run() {
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        web_sys::window()
            ...
            .and_then(|doc| {
                let dst = doc.get_element_by_id("tutowgpu")?;
        ...
        match event {
            Event::RedrawRequested(window_id) if window_id == state.window().id() => {
                ...
                match state.render() {
                    ...
                    // The system is out of memory, we should probab quit
